...
Results from the current evaluation show that the choice of operating system has the greatest impact on both $\delta$ and NRMSE (Figure 1) over compiler optimization strategies or type of build system used. A change in build generator to CMake appears to yield values of $\delta$ and NRMSE comparable, in terms of magnitude, to values of $\delta$ and NRMSE obtained from versions of WRF built with Makefiles[^2].
