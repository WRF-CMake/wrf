...
Table: Configurations used for build and integration tests. `Make`: original WRF build system files; `CMake`: this paper; Compiler-optimization: `Debug` disabled, `Release` enabled; Processor support: `serial` single, `dmpar` multiple with distributed memory (MPI), `smpar` multiple with shared memory (OpenMP), `dm_sm` multiple with MPI and OpenMP.
