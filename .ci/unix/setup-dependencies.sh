...
if [ "$(uname)" == "Linux" ]; then
    ...
    if [ "$(lsb_release -c -s)" == "trusty" ]; then
        ...
        if [ "$(lsb_release -c -s)" == "xenial" ]; then
            ...
            cd /tmp
            curl --retry ${HTTP_RETRIES} https://github.com/mdadams/jasper/archive/version-2.0.14.tar.gz | tar xz
        ...
        cd /tmp
        curl --retry ${HTTP_RETRIES} https://github.com/Unidata/netcdf-fortran/archive/v4.4.4.tar.gz | tar xz
    ...
    if [[ $MODE == dm* ]]; then
        if [ "$(lsb_release -c -s)" == "trusty" ]; then
            ...
            MPICH_VERSION=3.2.1
            cd /tmp
            curl --retry ${HTTP_RETRIES} https://www.mpich.org/static/downloads/${MPICH_VERSION}/mpich-${MPICH_VERSION}.tar.gz | tar xz
